...
export function EffectsLayer() {
  ...
  useEffect(() => {
    ...
    if (!pending) return;
    const glyph = new Text({ text: pending.emoji, style: { fontSize: 58, dropShadow: { alpha: .7, blur: 6, distance: 4 } } });
    glyph.anchor.set(.5);
    glyph.position.set(current.app.screen.width * .5, current.app.screen.height * .82);
    ...
    const tick = () => {
      ...
      glyph.x = current.app.screen.width * (.5 + .28 * progress);
      glyph.y = current.app.screen.height * (.82 - .54 * Math.sin(progress * Math.PI / 2));
