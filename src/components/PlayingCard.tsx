...
export function PlayingCard({ card, hidden = false, delay = 0, small = false }: { card?: Card; hidden?: boolean; delay?: number; small?: boolean }) {
  ...
  return (
    <motion.div className={`playing-card ${small ? "small" : ""} ${hidden ? "card-back" : ""} ${red ? "red" : ""}`} initial={{ opacity: 0, y: -28, rotate: -8 }} animate={{ opacity: 1, y: 0, rotate: 0 }} transition={{ delay, type: "spring", stiffness: 260, damping: 18 }}>
      {!hidden && card && <>
      ...
      {hidden && <span className="back-glyph">{"\u2660"}</span>}
    </motion.div>
  );
}
